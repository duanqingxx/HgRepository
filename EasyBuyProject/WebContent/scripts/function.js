        ...
        switch(name){
            case "userId":
                if(value == "") {
                    ...
                    errorMessage = "用户名由英文字母或数字6-10位";
                }else{
                	$.ajax({
                		url:"UserInfoServlet?type=checkName",
                		type:"post",
                		data:"userId="+value,
                		dataType:"text",
                		async:false,
...
//            location.href = "Cart?action=addAddress&address="+$("#addAddr").val();
            ...
            $("#span").append('<br/><input name="address" value='+addr+' type="radio" id="address0"/><span>'+addr+'</span>');
        }else{
    ...
    $(".manageDel").click(function(){
        ...
        if(confirm("确定要删除吗？")) {
            if($tr) $tr.remove();
//          alert($tr.children(":last").children(":last").attr("href"));
            var url=$tr.children(":last").children(":last").attr("url");
            location.href=url;
        }else{
            return false;
        }
    ...
    $(".number").find("span").click(function(){
        ...
        $number.val(number);
        var myprice=price * number;
        myprice=myprice.toFixed(2); 
        $priceBox.text("￥" +myprice);
        $("#shopping").find("#total").text("总计：￥"+totalPrice());
    });
    function myfunction() {
    	$(".number").each(function(){
    		var $tds=$(this).find("span").parent().parent().children("td");
            var $price= $($tds[1]);
            var $number=$($tds[2]);
            var price = $price.find("input[type='hidden']").val();//存值
            var $priceBox =$price.find("span");//现实价钱
    ...
    $("#logout").click(function(){
    	if(CarNum==0){
    		location.href="UserInfoServlet?type=logout";
    		return false;
    		}
    	else{
    ...
    $($(".MyDt")).nextUntil('dt').hide();
    $($(".MyDt")[0]).nextUntil('dt').show();
    //购物车
    var html1;
    $.getJSON("EProductBuyCarServlet?type=indexCar","",
    		function(json){
    	html1=new Array(json.length+1);
    	html1[0]=["<tr><th>图标</th><th>名称</th><th>价格</th></tr>"];
    	for (var i = 0; i < json.length; i++) {
    		html1[i+1]=["<tr><td>"+json[i].ep_id+"</td>" +
    				"<td>"+json[i].ep_name+"</td><td>"+
    				json[i].ep_price+"</td></tr>"];
