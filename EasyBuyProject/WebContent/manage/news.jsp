...
<div id="header" class="wrap">
	...
	<div class="navbar">
		<ul class="clearfix">
			...
			<li class="current"><a href="ENewsServlet?type=newsList">新闻</a></li>
		</ul>
	</div>
</div>
<div id="childNav">
...
<div id="main" class="wrap">
	...
	<div class="main">
		...
		<div class="manage">
			<table class="list">
				...
				<c:forEach items="${newsPage.newsList }" var="item" varStatus="status">
				<tr>
					<td class="first w4 c">${status.index+1 }</td>
					<td>${item.en_title }</td>
					<td class="w1 c">
					<a href="manage/news-modify.jsp?en_title=${item.en_title}
					&en_content=${item.en_content}&en_id=${item.en_id}">
					修改</a>
					<a class="manageDel" 
					href="#" url="ENewsServlet?type=delNews&en_id=${item.en_id}">删除</a>
					</td>
				</tr>
				</c:forEach>
    ...
    <div class="pager">
				<ul class="clearfix">
					<li><a href="ENewsServlet?type=newsList&pageIndex=1">首页</a></li>
					<c:if test="${newsPage.currPageNo==1 }">
					</c:if>
					<c:if test="${newsPage.currPageNo!=1 }">
					<li>...</li>
					<li><a href="ENewsServlet?type=newsList&pageIndex=${newsPage.currPageNo-1 }" >${newsPage.currPageNo-1 }</a></li>
					</c:if>
					<li class="current">${newsPage.currPageNo }</li>
					...
					<li><a href="ENewsServlet?type=newsList&pageIndex=${newsPage.totalPageCount }">尾页</a></li>
				</ul>
	</div>
</div>
<div id="footer">
